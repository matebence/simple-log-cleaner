...
class CleanLog(Clean):
    ...
    def remove_redundant_data_by(self, file_name):
        self.__files = Files()
        self.__files.set_extensions_source(file_name)
        return self
    ...
    def remove_http_requests_by(self, file_name):
        if self.__http is None:
            self.__http = Http()
        self.__http.set_request_source(file_name)
        return self

    def remove_robots_by(self, file_name):
        self.__robots = Robots()
        self.__robots.set_bots_source(file_name)
        return self
    ...
    def __delete_temp_file(self, temp_file_name):
        os.remove(temp_file_name) if pathlib.Path(temp_file_name).exists() else None
    ...
    def build(self):
        ...
        while line:
            ...
            if line == "" and self.__clean_up is False and self.__robots is not None:
                temp_file.close()
                ...
                line = self.__input_file.readline()
                items = Identify.items_from_log(line, self.__clean_up)
            elif line == "" and self.__clean_up is True:
                self.__output_file.close()
                self.__input_file.close()

                self.__delete_temp_file(Path.OUTPUT.value + "temp." + self.__output_file_name)
            elif line == "" and self.__clean_up is False and self.__robots is None:
                ...
                os.rename(Path.OUTPUT.value + "temp." + self.__output_file_name, Path.OUTPUT.value + self.__output_file_name)
            ...
            if items is not None:
                if self.__clean_up:
                    ...
                    if self.__files is not None and self.__files.get_extensions_source() is not None and self.__files.remove(items[Columns.REQUEST_URL.value]):
                        continue
                    if self.__http is not None and self.__http.get_status_source() is not None and self.__http.remove_statuses(items[Columns.HTTP_STATUS.value]):
                        continue
                    if self.__http is not None and self.__http.get_method_source() is not None and self.__http.remove_methods(items[Columns.REQUEST_METHOD.value]):
                        continue
                    if self.__http is not None and self.__http.get_request_source() is not None and self.__http.remove_request(items[Columns.REQUEST_URL.value]):
                        continue
                    if self.__robots is not None and self.__robots.black_list_ip(items[Columns.REQUEST_URL.value], items[Columns.IP_ADDRESS.value]):
                        continue
                    if self.__robots is not None and self.__robots.get_bots_source() is not None and self.__robots.remove_by_user_agent(items[Columns.USER_AGENT.value]):
                        continue
